    ...
    visitPhiValue = [this](PhiValue* value, uint32_t i, const std::vector<SectionCall*>& callVector)
        {
            ...
            std::cout << "bodyBranchPhi";
                ...
                {
                    // std::vector<bool> s1_br_map;
                    // std::vector<bool> s2_br_map;
                    // auto& result = summaryVector[i];
                    // result = summary1.copyBase();
                    // s1_br_map.resize(summary1.branches.size());
                    // s2_br_map.resize(summary2.branches.size());
                    // std::fill(s1_br_map.begin(), s1_br_map.end(), true);
                    // std::fill(s2_br_map.begin(), s2_br_map.end(), true);
                    
                    // for (int i1=0; i1 < summary1.branches.size(); i1++)
                    // {
                    //     const auto& branch1 = summary1.branches[i1];
                    //     bool is_tiled = false;
                    //     bool b2_handled = false;
                    //     for (const auto& index: branch1.index)
                    //     {
                    //         is_tiled = is_tiled || (has(index.func, "t") && has(index.func, "b_"));
                    //     }

                    //     for (int i2=0; i2 < summary2.branches.size(); i2++)
                    //     {
                    //         const auto& branch2 = summary2.branches[i2];
                    //         if ( is_tiled && result.canMerge(branch1, branch2)
                    //                     //   && proveFalse(branch1.cond && branch2.cond)
                    //                     //   && proveEquals(br_new.cond && summary0.base, br_new.cond)
                    //                            )
                    //         {
                    //             std::cout << "=====================\n";
                    //             auto cond1 = branch1.cond;
                    //             auto cond2 = branch2.cond;
                    //             auto merged = simplifyUseTactic(branch1.cond && branch2.cond);
                    //             std::cout << "cond1: \n" << cond1.to_string() << std::endl;
                    //             std::cout << "cond2: \n" << cond2.to_string() << std::endl;
                    //             std::cout << "merged cond:\n" << merged.to_string() << std::endl;
                    //             std::cout << "=====================\n";
                    //             if (!proveEquals(cond1 && summary0.base, cond1))
                    //             {
                    //                 s1_br_map[i1] = false;
                    //             }
                    //             if (!proveEquals(cond2 && !summary0.base, cond2))
                    //             {
                    //                 s2_br_map[i2] = false;
                    //             }
                    //         }
                    ...
                    // for (int i=0; i < summary1.branches.size(); ++i)
                    // {
                    //     if (s1_br_map[i]) result.addBranch(summary1.branches[i]);
                    // }
                    // for (int i=0; i < summary2.branches.size(); ++i)
                    // {
                    //     if (s2_br_map[i]) result.mergeBranch(summary2.branches[i]);
                    // }
                ...
                else
                {
                    ...
                    if (z3_sort)
                    {
                        if (z3_sort.is_array())
                        {
                            ...
                            assert(!summary1.branches.empty());
                            for (const auto& branch0 : summary0.branches)
                            {
                                for (const auto& branch1 : summary1.branches)
                                {
                                    for (const auto& branch2 : summary2.branches)
                                    {
                                        ...
                                        {
                                            if (!proveEquals(branch1.index[i].func, branch2.index[i].func))
                                            {
                                                if (branch1.index[i].skolem_b.has_value() && branch2.index[i].skolem_b.has_value())
                                                {
                                                    if (proveEquals(branch1.index[i].skolem_b.value(), branch2.index[i].skolem_b.value()))
                                                    {
                                                        std::cout << "Potential unrolling in branching\n";
                                                    }
                                                }
//                                                return false;
                                            }
                                    ...
                                    // do something when s2 is an original array
                                    if (summary2.branches.empty())
                                    {
                                        auto cond = simplifyUseTactic(branch0.cond && branch1.cond);
                                        if (!cond.is_false())
                                        {
                                            z3::expr_vector wi(this->z3ctx);
                                            for (const auto& i : branch1.index)
                                            {
                                                wi.push_back(i.func);
                                            }
                                            auto elem = z3::ite(branch0.elem, branch1.elem, this->z3ctx.real_val(0));
                                            summaryVector[i].mergeBranch({ cond, elem, branch1.layer, branch1.index });
                                        }
                                    }
                                }
            ...
            else if (auto loop = dynamic_cast<ParallelLoop*>(value->getSection()))
            {
                ...
                for (auto boundValue : loop->getBoundVector())
                {
                    ...
                    for (auto& branch : summary.branches)
                    {
                        for (uint32_t j = 0; j < branch.index.size(); j++)
                        {
                            ...
                            {
                                auto step = searchNonUnitStep(branch.index[j].func);
                                if (step > 0)
                                {
                                    sovleNonUnitAffine(branch.index[j].func, src_b, scale, offset, isAffine, isConstant, step);
                                }
                            }
                            if (isAffine && !isConstant)
                            {
                ...
                for (const auto& branch : init_summary.branches)
                {
                    ...
                    if (!remain_cond.is_false())
                    {
                        ...
                        z3::expr_vector dst_t(this->z3ctx);
                        auto sym_t = this->z3ctx.int_const("t");
                        src_t.push_back(sym_t);
                        dst_t.push_back(sym_t + 1);
...
//                            is_tiled = is_tiled || (t_aff && !t_cst && has(branch.elem, "w_"));
                            is_tiled = is_tiled || (has(ind.func, "b_") && has(ind.func, "t"));
                        }
                        ...
                        if (is_tiled)
                        {
                            remain_cond = simplifyUseTactic(remain_cond.substitute(src_t, dst_t));
                        }
