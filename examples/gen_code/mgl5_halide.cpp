...
Func targetFunction(const std::string &funcName,
    ...
    Func res(funcName);
    auto r_b = BoundaryConditions::constant_exterior(r, Expr(1.0));
    Func r_c("r_conv"), r1("r1"), r2("r2"), u_upd("u_upd");
    r_c(d1, d2, d3) = r_b(d1, d2, d3);

    r1(d1, d2, d3) = r_c(d1, d2-1, d3  ) + 
                     r_c(d1, d2+1, d3  ) + 
                     r_c(d1, d2  , d3-1) +
                     r_c(d1, d2  , d3+1);

    r2(d1, d2, d3) = r_c(d1, d2-1, d3-1) + 
                     r_c(d1, d2+1, d3-1) + 
                     r_c(d1, d2-1, d3+1) +
                     r_c(d1, d2+1, d3+1);
    
    u_upd(d1, d2, d3) = u(d1, d2, d3) +
                       Expr(c(0)) * r_c(d1, d2, d3) + 
                       Expr(c(1)) * (r_c(d1-1, d2, d3) + r_c(d1+1, d2, d3) + r1(d1, d2, d3)) + 
                       Expr(c(2)) * (r2(d1, d2, d3) + r1(d1-1, d2, d3) + r1(d1+1, d2, d3));
    Func u_fn("u");
    auto roi = i1s <= d1 and d1 <= i1e &&
               i2s <= d2 and d2 <= i2e &&
               i3s <= d3 and d3 <= i3e;
    u_fn(d1, d2, d3) = select(roi, u_upd(d1,d2,d3), u(d1,d2,d3));
...
extern "C" {
    void mg_loop5_(
        double* u, double* r, double *c, double *r1, double *r2,
...
int main(int argc, char** argv)
{
    ...
    const int d1_range = i1e - i1s + 1; // i1: 2 to 254 (253 elements)
    const int d2_range = i2e - i2s + 1; // i2: 2 to 254 (253 elements)  
    const int d3_range = i3e - i3s + 1; // i3: 2 to 254 (253 elements)
    
    // Array dimensions with padding for stencil access
    const int r_total_d1 = n1;
    const int r_total_d2 = n2;
    const int r_total_d3 = n3;
    const int u_total_d1 = n1;
    const int u_total_d2 = n2;
    const int u_total_d3 = n3;
    const int c_range = 3; // c(0:3)
    const int r1_range = 256;
    const int r2_range = 256;
    ...
    Buffer<double, 3> r({r_total_d1, r_total_d2, r_total_d3}, "r");
    Buffer<double, 3> u({u_total_d1, u_total_d2, u_total_d3}, "u");
    Buffer<double, 1> r1(r_total_d1, "fnm");
    Buffer<double, 1> r2(r_total_d1, "fnp");
    Buffer<double, 1> c(3, "c");
    Buffer<double> u_g(u), r_g(r), c_g(c);
    ...
    // --------------------------- mg_loop5 kernel --------------------------
    printf("mg_loop5 kernel start!\n");
    // gpu_target.set_feature(Halide::Target::Debug);

    i1_cpu.realize(c);
    auto res_cpu = targetFunction("mg_loop5_cpu",
                                  r, u, c,
                                  i1s, i1e,
                                  i2s, i2e,
                                  i3s, i3e);
    ...
    printf("mg_loop5 legacy code average cost time: %lfms\n\n",cost_time/times*1000);
    ...
    printf("mg_loop5 lifted cpu average cost time: %lfms\n\n",cost_time/times*1000);
    ...
    {
        i1_cpu.realize(c_g);
        auto res_gpu = targetFunction("mg_loop5_gpu",
                                    r_g, u_g, c_g,
                                    i1s, i1e,
                                    i2s, i2e,
                                    i3s, i3e);
