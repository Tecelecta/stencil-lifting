...
inline Func targetFunction(const std::string& funcName,
                           const Buffer<double, 3>& tmp1,
                           const Buffer<double, 3>& hat,
                           const Buffer<double, 2>& mm,
                           const Expr rdx,
                           ...
                           const Expr its, const Expr ite)
{
    ...
    target(i, k, j) = mm(i, j) * (rdx * (hat(2+i, k, 2+j+1) - hat(2+i, k, 2+j)) - tmp1(i, k, j));
    return target;
}
...
int main(int argc, char** argv)
{
    ...
    Buffer<double,3> tmp1({i_range+1, k_range+1, j_range+1}, "tmp1"), tmp1_g(tmp1);
    Buffer<double,3> hat({i_range+5, k_range+1, j_range+5}, "hat"), hat_g(hat);
    Buffer<double,2> mm({i_range+1, k_range+1}, "mm"), mm_g(mm);

    // baseline input
    Buffer<double,3> out_base({i_range+1, k_range+1, j_range+1}, "tmp1_");
    // halide cpu output
    Buffer<double,3> out_cpu({i_range+1, k_range+1, j_range+1}, "tmp1_");
    // halide gpu output
    Buffer<double,3> out_gpu({i_range+1, k_range+1, j_range+1}, "tmp1_");

    // --------------------------- advec_cell_kernel_loop91 kernel --------------------------
    printf("mdel13 kernel start!\n");
    ...
    // IO initialization
    init3_cpu.realize(tmp1);
    init3_cpu.realize(hat);
    init2_cpu.realize(mm);

    init3_cpu.realize(out_base);

    // Calling baseline
    double *hat_ = hat.get()->begin();
    double *mm_ = mm.get()->begin();
    ...
    for (int c = 0; c < j_range+1; c++) {
        for (int b = 0; b < k_range+1; b++) {
            for (int a = 0; a < i_range+1; a++) {
                double val_a = out_base(a,b,c);
                double val_b = out_cpu(a,b,c);
    ...
    if (with_gpu)
    {
        ...
        init3_gpu.realize(tmp1_g);
        init3_gpu.realize(hat_g);
        init2_gpu.realize(mm_g);
        
        try {
            // warmup
            init3_gpu.realize(out_gpu);
