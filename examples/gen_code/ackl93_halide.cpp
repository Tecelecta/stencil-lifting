...
void advec_cell_kernel_halide(Func& mass_flux_x_func, Func& ener_flux_func,
    ...
    auto vol_flux_positive = vol_flux_x(j_buf, k_buf) > Expr(0.0);
    
    auto upwind_buf = select(vol_flux_positive, j_buf-2, min(j_buf+1, x_max+2+j_offset));
    auto donor_buf = select(vol_flux_positive, j_buf-1, j_buf);
    auto downwind_buf = select(vol_flux_positive, j_buf, j_buf-1);
    auto dif_buf = select(vol_flux_positive, j_buf-1, min(j_buf+1, x_max+2+j_offset));
    
    auto sigmat = abs(vol_flux_x(j_buf, k_buf)) / pre_vol(donor_buf, k_buf);
    auto sigma3 = (Expr(1.0) + sigmat) * (vertexdx(j_buf) / vertexdx(dif_buf));
    auto sigma4 = Expr(2.0) - sigmat;
    
    auto sigma = sigmat;
    auto sigmav = sigmat;
    
    auto diffuw_density = density1(donor_buf, k_buf) - density1(upwind_buf, k_buf);
    auto diffdw_density = density1(downwind_buf, k_buf) - density1(donor_buf, k_buf);
    ...
    auto mass_flux_result = vol_flux_x(j_buf, k_buf) * (density1(donor_buf, k_buf) + limiter_density);
    mass_flux_x_func(j, k) = mass_flux_result;
    
    auto sigmam = abs(mass_flux_result) / (density1(donor_buf, k_buf) * pre_vol(donor_buf, k_buf));
    
    auto diffuw_energy = energy1(donor_buf, k_buf) - energy1(upwind_buf, k_buf);
    auto diffdw_energy = energy1(downwind_buf, k_buf) - energy1(donor_buf, k_buf);
    ...
    ener_flux_func(j, k) = mass_flux_result * (energy1(donor_buf, k_buf) + limiter_energy);
}

extern "C" {
    void advec_cell_kernel_(
        int* x_min, int* x_max, int* y_min, int* y_max,
        double* vertexdx, double* density1, double* energy1,
        double* mass_flux_x, double* vol_flux_x, double* pre_vol, double* ener_flux
    );
}
...
int main(int argc, char** argv)
{
    ...
    const int flux_x_size = (x_max + 3) - (x_min - 2) + 1;
    const int flux_y_size = (y_max + 2) - (y_min - 2) + 1;
    ...
    Buffer<double, 1> vertexdx({vertexdx_size}, "vertexdx"), vertexdx_g(vertexdx);
    ...
    Buffer<double, 2> vol_flux_x({flux_x_size, flux_y_size}, "vol_flux_x"), vol_flux_x_g(vol_flux_x);
    Buffer<double, 2> pre_vol({prevol_x_size, prevol_y_size}, "pre_vol"), pre_vol_g(pre_vol);

    Buffer<double, 2> mass_flux_x_base({flux_x_size, flux_y_size}, "mass_flux_x_base");
    Buffer<double, 2> ener_flux_base({prevol_x_size, prevol_y_size}, "ener_flux_base");
    
    // j: x_min to x_max+2 (x_max - x_min + 3), k: y_min to y_max (y_max - y_min + 1)
    const int output_j_size = x_max - x_min + 3;
    const int output_k_size = y_max - y_min + 1;
    
    Buffer<double, 2> mass_flux_x_cpu({output_j_size, output_k_size}, "mass_flux_x_cpu");
    Buffer<double, 2> ener_flux_cpu({output_j_size, output_k_size}, "ener_flux_cpu");
    
    Buffer<double, 2> mass_flux_x_gpu({output_j_size, output_k_size}, "mass_flux_x_gpu");
    Buffer<double, 2> ener_flux_gpu({output_j_size, output_k_size}, "ener_flux_gpu");
    
    printf("kernel start!\n");
    
    Func mass_flux_x_func_cpu("mass_flux_x_cpu");
    Func ener_flux_func_cpu("ener_flux_cpu");
    
    advec_cell_kernel_halide(mass_flux_x_func_cpu, ener_flux_func_cpu,
                            vertexdx, density1, energy1, vol_flux_x, pre_vol,
                            x_min, x_max, y_min, y_max);
    

    mass_flux_x_func_cpu.parallel(d2);
    if (density_x_size >= 8) mass_flux_x_func_cpu.vectorize(d1, 8);
    mass_flux_x_func_cpu.compile_jit(cpu_target);
    ...
    i1_cpu.realize(vertexdx);
    i2_cpu.realize(density1);
    i2_cpu.realize(energy1);
    i2_cpu.realize(vol_flux_x);
    i2_cpu.realize(pre_vol);
    
    z2_cpu.realize(mass_flux_x_base);
    ...
    advec_cell_kernel_(&x_min_i, &x_max_i, &y_min_i, &y_max_i,
                      ...
                      density1.get()->begin(),
                      energy1.get()->begin(),
                      mass_flux_x_base.get()->begin(),
                      vol_flux_x.get()->begin(),
                      pre_vol.get()->begin(),
                      ener_flux_base.get()->begin());
    
    try {
        z2_cpu.realize(mass_flux_x_cpu);
        z2_cpu.realize(ener_flux_cpu);
        
        mass_flux_x_func_cpu.realize(mass_flux_x_cpu);
    ...
    for (int k = y_min; k <= y_max; k++) {
        for (int j = x_min; j <= x_max+2; j++) {
            ...
            auto mass_diff = abs(mass_flux_x_base(fort_idx_j, fort_idx_k) - mass_flux_x_cpu(hal_idx_j, hal_idx_k));
            ...
            if (mass_diff >= 1e-10) {
                printf("mass_flux difference [%d, %d]: Fort=%lf | Hali=%lf\n", 
                       j, k, mass_flux_x_base(fort_idx_j, fort_idx_k), mass_flux_x_cpu(hal_idx_j, hal_idx_k));
            }
            if (ener_diff >= 1e-10) {
    ...
    if (with_gpu) {
        Func mass_flux_x_func_gpu("mass_flux_x_gpu");
        Func ener_flux_func_gpu("ener_flux_gpu");
        
        advec_cell_kernel_halide(mass_flux_x_func_gpu, ener_flux_func_gpu,
                                vertexdx_g, density1_g, energy1_g, vol_flux_x_g, pre_vol_g,
                                x_min, x_max, y_min, y_max);
        

        mass_flux_x_func_gpu.gpu_tile(d1, d2, d1o, d2o, d1i, d2i, 16, 16)
                            .compile_jit(gpu_target);
        ener_flux_func_gpu.gpu_tile(d1, d2, d1o, d2o, d1i, d2i, 16, 16)
                          .compile_jit(gpu_target);
            
        i1_gpu.realize(vertexdx_g);
        i2_gpu.realize(density1_g);
        i2_gpu.realize(energy1_g);
        i2_gpu.realize(vol_flux_x_g);
        i2_gpu.realize(pre_vol_g);
        
        try {
            z2_gpu.realize(mass_flux_x_gpu);
            z2_gpu.realize(ener_flux_gpu);
            mass_flux_x_func_gpu.realize(mass_flux_x_gpu);
