...
void pdv_kernel2_halide(Func& density1_func, Func& energy1_func,
    ...
    auto left_flux = (xarea(j_buf, k_buf) * (xvel0(j_buf, k_buf) + xvel0(j_buf, k_buf+1) + xvel1(j_buf, k_buf) + xvel1(j_buf, k_buf+1))) * Expr(0.25) * Expr(dt);
    auto right_flux = (xarea(j_buf+1, k_buf) * (xvel0(j_buf+1, k_buf) + xvel0(j_buf+1, k_buf+1) + xvel1(j_buf+1, k_buf) + xvel1(j_buf+1, k_buf+1))) * Expr(0.25) * Expr(dt);
    auto bottom_flux = (yarea(j_buf, k_buf) * (yvel0(j_buf, k_buf) + yvel0(j_buf+1, k_buf) + yvel1(j_buf, k_buf) + yvel1(j_buf+1, k_buf))) * Expr(0.25) * Expr(dt);
    auto top_flux = (yarea(j_buf, k_buf+1) * (yvel0(j_buf, k_buf+1) + yvel0(j_buf+1, k_buf+1) + yvel1(j_buf, k_buf+1) + yvel1(j_buf+1, k_buf+1))) * Expr(0.25) * Expr(dt);
...
int main(int argc, char** argv)
{
    ...
    Buffer<double, 2> xvel1({xarea_x_size, yarea_y_size}, "xvel1"), xvel1_g(xvel1);
    Buffer<double, 2> yvel0({xarea_x_size, yarea_y_size}, "yvel0"), yvel0_g(yvel0);
    Buffer<double, 2> yvel1({xarea_x_size, yarea_y_size}, "yvel1"), yvel1_g(yvel1);
    ...
    pdv_kernel2_halide(density1_func_cpu, energy1_func_cpu,
                      xarea, yarea, volume, density0, energy0, pressure, viscosity, xvel0, xvel1, yvel0, yvel1, dt,
    ...
    i2_cpu.realize(xvel1);
    i2_cpu.realize(yvel0);
    i2_cpu.realize(yvel1);
    ...
    pdv_kernel_(&x_min_i, &x_max_i, &y_min_i, &y_max_i, &dt_i,
                ...
                xvel0.get()->begin(), xvel1.get()->begin(),
                yvel0.get()->begin(), yvel1.get()->begin());
    ...
    if (with_gpu) {
        ...
        pdv_kernel2_halide(density1_func_gpu, energy1_func_gpu,
                          xarea_g, yarea_g, volume_g, density0_g, energy0_g, pressure_g, viscosity_g, xvel0_g, xvel1_g, yvel0_g, yvel1_g, dt,
                          x_min, x_max, y_min, y_max);
        ...
        i2_gpu.realize(xvel1_g);
        i2_gpu.realize(yvel0_g);
        i2_gpu.realize(yvel1_g);
