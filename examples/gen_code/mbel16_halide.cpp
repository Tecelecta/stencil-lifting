...
inline Func init_nonzero<3>(const std::string& funcName, bool vectorize)
{
    Var d1, d2, d3;
    Func nz(funcName);
    nz(d1, d2, d3) = cast<float>(((((d1 + d2*10 + d3*100)) + Expr(1e-5)) / Expr(10000000.0)));
    return nz;
}
...
void targetFunction(Func& bdy_fn,
                    ...
                    Buffer<float, 2> mub,
                    Buffer<float, 3> mu_bdy_xe,
                    Buffer<float, 3> mu_bdy_tend_xe,
                    Buffer<float, 3> moist_bdy_xe,
                    Buffer<float, 3> moist_bdy_tend_xe,
                    Buffer<float, 3> t_bdy_xe, 
                    Buffer<float, 3> t_bdy_tend_xe) {
    ...
    auto mu_old_bdy_xe    = mu_bdy_xe(i,0,j) + mub(i,j);
    auto t_old_bdy_xe     = t_bdy_xe(i,k,j) / mu_old_bdy_xe;
    auto moist_old_bdy_xe = moist_bdy_xe(i,k,j) / mu_old_bdy_xe;
    auto mu_new_bdy_xe    = mu_old_bdy_xe + mu_bdy_tend_xe(i,0,j) * dt_interval;
    auto t_new_bdy_xe     = (t_bdy_xe(i,k,j) + t_bdy_tend_xe(i,k,j) * dt_interval ) / mu_new_bdy_xe;
    auto moist_new_bdy_xe = (moist_bdy_xe(i,k,j) + moist_bdy_tend_xe(i,k,j)*dt_interval) / mu_new_bdy_xe;
    
    auto bdy_tval = ( ( ( t_old_bdy_xe + T0 ) * ( Expr(1.f) + (R_v/R_d) * moist_old_bdy_xe ) ) - T0 ) * mu_old_bdy_xe;
    auto bdy_tend_tval = ( ( mu_new_bdy_xe * ( ( t_new_bdy_xe + T0 ) * ( Expr(1.f) + (R_v/R_d) * moist_new_bdy_xe ) - T0 ) ) - 
                           ( mu_old_bdy_xe * ( ( t_old_bdy_xe + T0 ) * ( Expr(1.f) + (R_v/R_d) * moist_old_bdy_xe ) - T0 ) ) ) / dt_interval;
    auto bdy_fval = ( ( ( t_old_bdy_xe + T0 ) / ( Expr(1.f) + (R_v/R_d) * moist_old_bdy_xe ) ) - T0 ) * mu_old_bdy_xe;
    auto bdy_tend_fval = ( ( mu_new_bdy_xe * ( ( t_new_bdy_xe + T0 ) / ( Expr(1.f) + (R_v/R_d) * moist_new_bdy_xe ) - T0 ) ) - 
                           ( mu_old_bdy_xe * ( ( t_old_bdy_xe + T0 ) / ( Expr(1.f) + (R_v/R_d) * moist_old_bdy_xe ) - T0 ) ) ) / dt_interval;
...
extern "C" {
    void theta_and_thetam_lbc_only_(
        float* new_t_bdy_xe, float * new_t_bdy_tend_xe, bool* theta_to_thetam,
        const float* mub, const float* mu_bdy_xe, const float* mu_bdy_tend_xe, 
        const float *moist_bdy_xe, const float* moist_bdy_tend_xe, 
        const float* t_bdy_xe, const float *t_bdy_tend_xe, 
...
int main(int argc, char** argv)
{
    ...
    // Extended ranges for arraxe with padding
    ...
    Buffer<float, 3> mu_bdy_xe({in_d1_range, 1, in_d3_range}, "mu_bdy_xe"), mu_bdy_xe_g(mu_bdy_xe),
                     mu_bdy_tend_xe({in_d1_range, 1, in_d3_range}, "mu_bdy_tend_xe"), mu_bdy_tend_xe_g(mu_bdy_tend_xe),
                     moist_bdy_xe({in_d1_range, in_d2_range, in_d3_range}, "moist_bdy_xe"), moist_bdy_xe_g(moist_bdy_xe),
                     moist_bdy_tend_xe({in_d1_range, in_d2_range, in_d3_range}, "moist_bdy_tend_xe"), moist_bdy_tend_xe_g(moist_bdy_tend_xe),
                     t_bdy_xe({in_d1_range, in_d2_range, in_d3_range}, "t_bdy_xe"), t_bdy_xe_g(t_bdy_xe),
                     t_bdy_tend_xe({in_d1_range, in_d2_range, in_d3_range}, "t_bdy_tend_xe"), t_bdy_tend_xe_g(t_bdy_tend_xe);
    ...
    Func bdy_fn_cpu("new_t_bdy_xe"), bdy_tend_fn_cpu("new_t_bdy_tend_xe");
    ...
    try {
    i3_cpu.realize(mu_bdy_xe);
    i3_cpu.realize(mu_bdy_tend_xe);
    i3_cpu.realize(moist_bdy_xe);
    i3_cpu.realize(moist_bdy_tend_xe);
    i3_cpu.realize(t_bdy_xe);
    i3_cpu.realize(t_bdy_tend_xe);
    i2_cpu.realize(mub);
    ...
    theta_and_thetam_lbc_only_(
        ...
        mub.get()->begin(),
        mu_bdy_xe.get()->begin(),
        mu_bdy_tend_xe.get()->begin(),
        moist_bdy_xe.get()->begin(),
        moist_bdy_tend_xe.get()->begin(),
        t_bdy_xe.get()->begin(),
        t_bdy_tend_xe.get()->begin(),
        &its, &ite,
        &kts, &kte,
        &jts, &jte
    );
    ...
    if ( with_gpu )
    {
        Func bdy_fn_gpu("new_t_bdy_xe"), bdy_tend_fn_gpu("new_t_bdy_tend_xe");
        ...
        // IO initialization
        i3_gpu.realize(mu_bdy_xe_g);
        i3_gpu.realize(mu_bdy_tend_xe_g);
        i3_gpu.realize(moist_bdy_xe_g);
        i3_gpu.realize(moist_bdy_tend_xe_g);
        i3_gpu.realize(t_bdy_xe_g);
        i3_gpu.realize(t_bdy_tend_xe_g);
