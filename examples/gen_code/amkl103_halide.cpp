...
extern "C" {
    void advec_mom_kernel_loop1y_min3_(double *node_flux,
...
int main(int argc, char** argv)
{
    ...
    Buffer<double,2> node_flux({x_range + 6, y_range + 6}, "node_flux"), node_flux_g(node_flux);
    Buffer<double,2> node_mass_post({x_range + 6, y_range + 6}, "node_mass_post"), node_mass_post_g(node_mass_post);

    Buffer<double,2> node_mass_pre_base({x_range + 6, y_range + 6}, "node_mass_pre");
    
    Buffer<double,2> node_mass_pre_cpu({x_range + 4, y_range + 2}, "node_mass_pre");

    Buffer<double,2> node_mass_pre_gpu({x_range + 4, y_range + 2}, "node_mass_pre");

    // --------------------------- advec_mom_kernel_loop1y_min3 kernel --------------------------
    // printf("advec_mom_kernel_loop1y_min3 kernel start!\n");

    Func cpu_fn("cpu_fn");
    cpu_fn(j, k) = node_mass_post(j + 1, k + 2) - node_flux(j, k + 2) + node_flux(j + 1, k + 2);
    cpu_fn.parallel(k);
    if (x_range + 4 >= 8) cpu_fn.vectorize(j, 8);
    cpu_fn.compile_jit(cpu_target);


    for (int y = 0; y < node_flux.height(); y++) {
        for (int x = 0; x < node_flux.width(); x++) {
            node_flux(x, y) = ((((x + y) % 13) + 0.1) / 13.0) * 10.0;
        }
    }
    
    for (int y = 0; y < node_mass_post.height(); y++) {
        for (int x = 0; x < node_mass_post.width(); x++) {
            node_mass_post(x, y) = ((((x + y) % 13) + 0.2) / 13.0) * 10.0;
        }
    }

    zero_cpu.realize(node_mass_pre_base);

    // Calling baseline Fortran
    double *node_flux_ = node_flux.get()->begin();
    double *node_mass_post_ = node_mass_post.get()->begin();
    double *node_mass_pre_ = node_mass_pre_base.get()->begin();
    
    advec_mom_kernel_loop1y_min3_(node_flux_, node_mass_post_, node_mass_pre_, 
                                  &x_max, &x_min, &y_max, &y_min);
    
    // Calling halide cpu
    try {
        zero_cpu.realize(node_mass_pre_cpu);
        cpu_fn.realize(node_mass_pre_cpu);
    ...
    for (int k_idx = 0; k_idx < y_range + 2; k_idx++) {
        for (int j_idx = 0; j_idx < x_range + 4; j_idx++) {
            ...
            double diff = abs(node_mass_pre_base(cpp_j, cpp_k) - node_mass_pre_cpu(j_idx, k_idx));
            
            // if (diff >= 1e-10)
            // {
            //     if (errors < 10) {
            //         printf("node_mass_pre [%d, %d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
            //                 j_idx, k_idx, node_mass_pre_base(cpp_j, cpp_k), node_mass_pre_cpu(j_idx, k_idx), diff);
            //     }
            //     errors++;
            // }
            correctness += diff;
    ...
    if (with_gpu)
    {
        // building gpu func
        Func gpu_fn("gpu_fn");
        gpu_fn(j, k) = node_mass_post_g(j + 1, k + 2) - node_flux_g(j, k + 2) + node_flux_g(j + 1, k + 2);
        gpu_fn.gpu_tile(j, k, bj, bk, tj, tk, 8, 8)
              .compile_jit(gpu_target);
        
        // GPU IO initialization
        node_flux_g.copy_from(node_flux);
        node_mass_post_g.copy_from(node_mass_post);
        
        // warmup
        try {
            zero_gpu.realize(node_mass_pre_gpu);
            gpu_fn.realize(node_mass_pre_gpu);
        } catch ( RuntimeError &e ) {
            std::cerr << e.what();
            return -1;
        }
