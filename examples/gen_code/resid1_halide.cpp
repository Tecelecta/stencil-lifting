...
Func targetFunction(const std::string &funcName,
    ...
    r_upd(d1, d2, d3) = v(d1, d2, d3) -
                       Expr(-1.4) * u_c(d1, d2, d3) - 
                       Expr(1.26) * (u2(d1, d2, d3) + u1(d1-1, d2, d3) + u1(d1+1, d2, d3)) -
                       Expr(-1.53) * (u2(d1-1, d2, d3) + u2(d1+1, d2, d3));
    Func r_fn("r");
...
extern "C" {
    void resid_(
        double* r, double* u, double *v, double *u1, double *u2,
...
int main(int argc, char** argv)
{
    ...
    Buffer<double> u_g(u), r_g(r), v_g(v);
    ...
    // --------------------------- resid kernel --------------------------
    printf("resid1 kernel start!\n");
    // gpu_target.set_feature(Halide::Target::Debug);

    auto res_cpu = targetFunction("resid_cpu",
                                  r, u, v,
                                  i1s, i1e,
                                  i2s, i2e,
                                  i3s, i3e);
    ...
    printf("resid1 legacy code average cost time: %lfms\n\n",cost_time/times*1000);
    ...
    printf("resid1 lifted cpu average cost time: %lfms\n\n",cost_time/times*1000);



    if ( with_gpu )
    {
        auto res_gpu = targetFunction("resid_gpu",
                                    r_g, u_g, v_g,
                                    i1s, i1e,
                                    i2s, i2e,
                                    i3s, i3e);
