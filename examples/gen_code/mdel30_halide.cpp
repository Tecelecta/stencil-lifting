...
Func targetFunction(Buffer<double, 2> rmol,
                    Buffer<double, 3> za) {

    Var i("i"), k("k"), j("j");

    Func res("meso_length_scale");
    Expr karman(.4), alp4(100.);

    auto brval_t = karman*za(i,k,j)/(Expr(1.)+Expr(2.7)*min(za(i,k,j)*rmol(i,j),Expr(1.)));

    auto coe = pow((Expr(1.) - alp4*za(i,k,j)*rmol(i,j)), Expr(.2));
    auto brval_f = Expr(1.) * karman*za(i,k,j) * coe;
...
int main(int argc, char** argv)
{
    ...
    Buffer<double, 2> rmol({in_d1_range, in_d3_range}, "rmol"), rmol_g(rmol);
    Buffer<double, 3> za({in_d1_range, in_d2_range, in_d3_range}, "za"), za_g(za);

    // Fortran output
    Buffer<double, 3> els_base({out_d1_range, out_d2_range, out_d3_range}, "els_raw"); // els

    // Halide CPU output
    Buffer<double, 3> els_cpu({out_d1_range, out_d2_range, out_d3_range}, "els"); // els

    // Halide GPU roiput
    Buffer<double, 3> els_gpu({out_d1_range, out_d2_range, out_d3_range}, "els"); // els

    // --------------------------- kernel --------------------------
    printf("kernel start!\n");

    // Coordinate transformations for proper indexing
    auto res_cpu = targetFunction(rmol, za);
    ...
    // CPU execution
    try {
        z3_cpu.realize(els_cpu);
        res_cpu.realize(els_cpu);
    ...
    for (int j = 0; j < out_d3_range; j++) {
        for (int k = 0; k < out_d2_range; k++) {
            for (int i = 0; i < out_d1_range; i++) {
                ...
                int b = k;
                int c = j;
                auto A = els_base(a,b,c);
                auto B = els_cpu(i,k,j);
    ...
    if ( with_gpu )
    {
        auto res_gpu = targetFunction(rmol_g, za_g);
        ...
        // IO initialization
        i2_gpu.realize(rmol_g);
        i3_gpu.realize(za_g);

        // GPU warm up
        try {
            z3_gpu.realize(els_gpu);
            res_gpu.realize(els_gpu);
        } catch ( RuntimeError &e ) {
            std::cerr << e.what();
            return 2;
        }
