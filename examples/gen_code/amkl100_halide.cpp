...
extern "C" {
    void advec_mom_kernel_loop100_(int *j,
                                   int *k,
                                   double *post_vol,
                                   double *pre_vol, 
                                   double *vol_flux_x,
...
int main(int argc, char** argv)
{
    ...
    // --------------------------- advec_mom_kernel_loop100 kernel --------------------------
    // printf("advec_mom_kernel_loop100 kernel start!\n");

    // post_vol(j,k) = volume(j,k)
    Func cpu_fn_post("cpu_fn_post");
    cpu_fn_post(j, k) = volume(j, k);
    ...
    advec_mom_kernel_loop100_(&j_dummy, &k_dummy,
                              post_vol_, pre_vol_, 
                              vol_flux_x_, volume_, 
