...
void advec_mom_kernel_loop108_halide(Func& mom_flux_func,
    ...
    // amkl108: j=x_min,x_max+1; k=y_min-1,y_max+1
    // j=0 -> Fortran j=x_min -> array index x_min-(x_min-2) = 2
    // k=0 -> Fortran k=y_min-1 -> array index (y_min-1)-(y_min-2) = 1
    Expr j_offset = Expr(2);  // j=0 -> Fortran j=x_min -> array index 2
    Expr k_offset = Expr(1);  // k=0 -> Fortran k=y_min-1 -> array index 1
    ...
    auto upwind_buf = select(node_flux_negative, 
                            min(k_buf+2, Expr(y_max+3+k_offset-1)), 
                            max(k_buf-1, Expr(y_min-2+k_offset)));
    auto donor_buf = select(node_flux_negative, k_buf+1, k_buf);
    auto downwind_buf = select(node_flux_negative, k_buf, k_buf+1);
    auto dif_buf = select(node_flux_negative, k_buf+1, 
                         max(k_buf-1, Expr(y_min-2+k_offset)));
    
    auto sigma = abs(node_flux(j_buf, k_buf)) / node_mass_pre(j_buf, donor_buf);
    auto width = celldy(k_buf);
    
    auto vdiffuw = vel1(j_buf, donor_buf) - vel1(j_buf, upwind_buf);
    auto vdiffdw = vel1(j_buf, downwind_buf) - vel1(j_buf, donor_buf);
    ...
    auto limiter = select(limiter_condition,
        wind * min(width * ((Expr(2.0) - sigma) * adw / width + (Expr(1.0) + sigma) * auw / celldy(dif_buf)) / Expr(6.0),
                   min(auw, adw)),
        Expr(0.0));
    
    auto advec_vel_s = vel1(j_buf, donor_buf) + (Expr(1.0) - sigma) * limiter;
    
    auto is_boundary = k == Expr(0);
...
extern "C" {
    void advec_mom_kernel_loop108_(
...
int main(int argc, char** argv)
{
    ...
    const int celldy_size = array_y_size;
    ...
    auto i1_cpu = init_nonzero<1>("init1d", array_y_size >= 8);
    ...
    advec_mom_kernel_loop108_halide(mom_flux_func_cpu,
                                   node_flux, node_mass_pre, celldy, vel1,
    ...
    advec_mom_kernel_loop108_(mom_flux_f.data(),
                             node_flux_f.data(),
    ...
    for (int i = 0; i < times; i++) {
        ...
        advec_mom_kernel_loop108_(mom_flux_base.get()->begin(),
                                 node_flux.get()->begin(),
    ...
    if (with_gpu) {
        ...
        advec_mom_kernel_loop108_halide(mom_flux_func_gpu,
                                       node_flux_g, node_mass_pre_g, celldy_g, vel1_g,
                                       x_min, x_max, y_min, y_max);
