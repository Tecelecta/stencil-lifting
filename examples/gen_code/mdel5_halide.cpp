...
inline Func targetFunction(const std::string& funcName,
                           Buffer<double, 3>& tmp1,
                           const Buffer<double, 3>& zx,
                           const Buffer<double, 3>& hatavg,
                           const Buffer<double, 3>& rdzw,
                           ...
                           const Expr its, const Expr ite)
{
    ...
    auto tmpzx = Expr(0.25) * (
                    zx(2+i, k,   2+j) + zx(2+i+1, k,   2+j) + 
                    zx(2+i, k+1, 2+j) + zx(2+i+1, k+1, 2+j) );
    target(i, k, j) = (hatavg(2+i, k+1, 2+j) - hatavg(2+i, k, 2+j)) * tmpzx * rdzw(i, k, j);
    return target;
}

extern "C" {
    void cal_deform_and_div_(double *tmp1, 
                             double *zx, 
                             double *hatavg,
                             double *rdzw,
...
int main(int argc, char** argv)
{
    ...
    Buffer<double,3> zx({i_range+5, k_range+1, j_range+5}, "zx"), zx_g(zx);
    Buffer<double,3> hatavg({i_range+5, k_range+1, j_range+5}, "hatavg"), hatavg_g(hatavg);
    Buffer<double,3> rdzw({i_range+1, k_range+1, j_range+1}, "rdzw"), rdzw_g(rdzw);

    // baseline input
    Buffer<double,3> out_base({i_range+1, k_range+1, j_range+1}, "tmp1");
    // halide cpu output
    Buffer<double,3> out_cpu({i_range+1, k_range, j_range+1}, "tmp1");
    // halide gpu output
    Buffer<double,3> out_gpu({i_range+1, k_range, j_range+1}, "tmp1");

    // --------------------------- advec_cell_kernel_loop91 kernel --------------------------
    printf("mdel5 kernel start!\n");

    // building baseline stencil
    Func cpu_fn = targetFunction("cal_deform_and_div_cpu", 
                                out_cpu, zx, hatavg, rdzw, 
                                Expr(jts), Expr(jte), 
                                Expr(kts), Expr(kte), 
                                Expr(its), Expr(ite));
    cpu_fn.parallel(j);
    ...
    init3_cpu.realize(hatavg);
    init3_cpu.realize(rdzw);

    zero_cpu.realize(out_base);

    // Calling baseline
    double *zx_ = zx.get()->begin();
    double *hatavg_ = hatavg.get()->begin();
    double *rdzw_ = rdzw.get()->begin();
    double *tmp1_ = out_base.get()->begin();
    cal_deform_and_div_(tmp1_, 
                        zx_, 
                        hatavg_, 
                        rdzw_, 
            ...
            for (int a = 0; a < i_range+1; a++) {
                ...
                if ( diff >= 1e-3 )
                {
                    printf("output1 [%d, %d, %d] = A: %lf | B: %lf\n", 
                            a, b, c, out_base(a,b,c), out_cpu(a,b,c));
                }
                correctness_1 += diff;
            }
        }
    ...
    {
        // building gpu func
        Func gpu_fn = targetFunction("cal_deform_and_div_cpu", 
                                    out_gpu, zx_g, hatavg_g, rdzw_g, 
                                    Expr(jts), Expr(jte), 
                                    Expr(kts), Expr(kte), 
                                    Expr(its), Expr(ite));
        ...
        init3_gpu.realize(hatavg_g);
        init3_gpu.realize(rdzw_g);
