...
Func targetFunction(const std::string &funcName,
    ...
    auto roi2 = i1e == d1 &&
            i2s <= d2 and d2 <= i2e &&
            i3s <= d3 and d3 <= i3e;
    res(d1, d2, d3) = select(roi1, u(i1e-1, d2, d3),
                             select(roi2, u(1, d2, d3),
                                    u(d1, d2, d3)));
...
int main(int argc, char** argv)
{
    ...
    const int i1s = 0, i1e = n1 - 1; // i1: 2 to 254 for u computation
    ...
    // --------------------------- mg_loop11 kernel --------------------------
    printf("mg_loop11 kernel start!\n");
    // gpu_target.set_feature(Halide::Target::Debug);

    auto res_cpu = targetFunction("mg_loop11_cpu",
                                  u,
                                  i1s, i1e,
                                  i2s, i2e,
                                  i3s, i3e);
    ...
    printf("mg_loop11 legacy code average cost time: %lfms\n\n",cost_time/times*1000);
    ...
    printf("mg_loop11 lifted cpu average cost time: %lfms\n\n",cost_time/times*1000);



    if ( with_gpu )
    {
        auto res_gpu = targetFunction("mg_loop11_gpu",
                                    u_g,
                                    i1s, i1e,
                                    i2s, i2e,
                                    i3s, i3e);
