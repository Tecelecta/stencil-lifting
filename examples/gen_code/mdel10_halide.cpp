                           ...
                           const Expr its, const Expr ite)
{
    ...
    target(i, k, j) = select(roi, 
        Expr(0.5) * ( fnm(k+1) * ( hat(i, k+1, j) + hat(i, k+1, j+1)) + 
                      fnp(k+1) * ( hat(i, k,   j) + hat(i, k,   j+1)) ),
        hatavg(i, k, j));
    return target;
}
...
int main(int argc, char** argv)
{
    ...
    // halide cpu output
    Buffer<double,3> out_cpu({i_range+5, k_range, j_range+4}, "hatavg");
    // halide gpu output
    Buffer<double,3> out_gpu({i_range+5, k_range, j_range+4}, "hatavg");

    // --------------------------- advec_cell_kernel_loop91 kernel --------------------------
    printf("mdel10 kernel start!\n");
