...
// miel13 kernel
Func targetFunction(Buffer<float, 3> v_old,
                   Buffer<float, 3> tendency,
                   Buffer<float, 3> rom,
                   Buffer<float, 2> muv_old,
                   Buffer<float, 2> muv_new,
                   Buffer<float, 1> c1,
                   Buffer<float, 1> c2,
                   Buffer<float, 2> msfvy,
                   Buffer<float, 2> msfvx,
                   Buffer<float, 1> rdzw) {
    Var i("i"), k("k"), j("j");
    ...
    rt(i,k,j) = dt_rk * tendency(i,k,j) * msfvx(i,j)
                - (c1(k)*muv_old(i,j)+c2(k))*(at*v_old(i,km1,j) + btmp*v_old(i,k,j) + ct*v_old(i,kp1,j));
    return rt;
}
...
extern "C" {
    void advect_v_implicit_(
        float* rt,
        const float* v_old, const float* tendency, const float* rom,
        const float* c1, const float* c2,
        const float* muv_old, const float* muv_new,
        const float* msfvy, const float* msfvx, const float* rdzw,
...
int main(int argc, char** argv)
{
    ...
    Buffer<float, 1> rdzw(in_d2_range, "rdzw"), rdzw_g(rdzw);
    Buffer<float, 1> c1(in_d2_range, "c1"), c1_g(c1),
                     c2(in_d2_range, "c2"), c2_g(c2);

    Buffer<float, 2> msfvy({in_d1_range, in_d3_range}, "msfvy"), msfvy_g(msfvy),
                     msfvx({in_d1_range, in_d3_range}, "msfvx"), msfvx_g(msfvx),
                     muv_old({in_d1_range, in_d3_range}, "muv_old"), muv_old_g(muv_old),
                     muv_new({in_d1_range, in_d3_range}, "muv_new"), muv_new_g(muv_new);
    
    Buffer<float, 3> v_old({in_d1_range, in_d2_range, in_d3_range}, "v_old"), v_old_g(v_old),
                     tendency({in_d1_range, in_d2_range, in_d3_range}, "tendency"), tendency_g(tendency),
                     rom({in_d1_range, in_d2_range+1, in_d3_range+1}, "rom"), rom_g(rom);
    ...
    // --------------------------- miel13 kernel --------------------------
    printf("miel13 kernel start!\n");

    auto res_cpu = targetFunction(v_old, tendency, rom, muv_old, muv_new, c1, c2, msfvy, msfvx, rdzw);
    ...
    try {
    i3_cpu.realize(v_old);
    i3_cpu.realize(tendency);
    i3_cpu.realize(rom);
    i2_cpu.realize(msfvy);
    i2_cpu.realize(msfvx);
    i2_cpu.realize(muv_old);
    i2_cpu.realize(muv_new);
    i1_cpu.realize(rdzw);
    i1_cpu.realize(c1);
    i1_cpu.realize(c2);
    ...
    if ( with_gpu )
    {
        auto res_gpu = targetFunction(v_old_g, tendency_g, rom_g, muv_old_g, muv_new_g, c1_g, c2_g, msfvy_g, msfvx_g, rdzw_g);
        res_gpu.gpu_tile(d1,d2,d3,d1o,d2o,d3o,d1i,d2i,d3i,8,8,4)
                .compile_jit(gpu_target);
        i3_gpu.realize(v_old_g);
        i3_gpu.realize(tendency_g);
        i3_gpu.realize(rom_g);
        i2_gpu.realize(msfvy_g);
        i2_gpu.realize(msfvx_g);
        i2_gpu.realize(muv_old_g);
        i2_gpu.realize(muv_new_g);
