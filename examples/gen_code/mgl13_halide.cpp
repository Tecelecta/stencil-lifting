...
Func targetFunction(const std::string &funcName,
    ...
    auto roi2 = i3e == d3 &&
                i1s <= d1 and d1 <= i1e &&
                i1s <= d1 and d1 <= i1e;
    res(d1, d2, d3) = select(roi1, u(d1, d2, i3e-1),
                             select(roi2, u(d1, d2, 1),
                                    u(d1, d2, d3)));
...
int main(int argc, char** argv)
{
    ...
    const int i3s = 0, i3e = n3 - 1; // i3: 2 to 254
    ...
    // --------------------------- mg_loop13 kernel --------------------------
    printf("mg_loop13 kernel start!\n");
    // gpu_target.set_feature(Halide::Target::Debug);

    auto res_cpu = targetFunction("mg_loop13_cpu",
                                  u,
                                  i1s, i1e,
                                  i2s, i2e,
                                  i3s, i3e);
    ...
    printf("mg_loop13 legacy code average cost time: %lfms\n\n",cost_time/times*1000);
    ...
    printf("mg_loop13 lifted cpu average cost time: %lfms\n\n",cost_time/times*1000);



    if ( with_gpu )
    {
        auto res_gpu = targetFunction("mg_loop13_gpu",
                                    u_g,
                                    i1s, i1e,
                                    i2s, i2e,
                                    i3s, i3e);
