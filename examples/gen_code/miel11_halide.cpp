...
// miel11 kernel
Func targetFunction(Buffer<float, 3> s_old,
    ...
    auto wiL = rom(i,k,j) * rdzw(k) / (c1(k)*mut_new(i,j)+c2(k));
    auto wiR = rom(i,k+1,j) * rdzw(k) / (c1(k)*mut_new(i,j)+c2(k));
    ...
    rt(i,k,j) = dt_rk * tendency(i,k,j)
                - (c1(k)*mut_old(i,j)+c2(k))*(at*s_old(i,km1,j) + btmp*s_old(i,k,j) + ct*s_old(i,kp1,j));
    return rt;
}

extern "C" {
    void advect_s_implicit_(
        float* rt,
        const float* s_old, const float* tendency, const float* rom,
...
int main(int argc, char** argv)
{
    ...
    Buffer<float, 2> mut_old({in_d1_range, in_d3_range}, "mut_old"), mut_old_g(mut_old),
                     mut_new({in_d1_range, in_d3_range}, "mut_new"), mut_new_g(mut_new);
    
    Buffer<float, 3> s_old({in_d1_range, in_d2_range, in_d3_range}, "s_old"), s_old_g(s_old),
    ...
    // --------------------------- miel11 kernel --------------------------
    printf("miel11 kernel start!\n");

    auto res_cpu = targetFunction(s_old, tendency, rom, mut_old, mut_new, c1, c2, rdzw);
    ...
    if ( with_gpu )
    {
        auto res_gpu = targetFunction(s_old_g, tendency_g, rom_g, mut_old_g, mut_new_g, c1_g, c2_g, rdzw_g);
        res_gpu.gpu_tile(d1,d2,d3,d1o,d2o,d3o,d1i,d2i,d3i,8,8,4)
                .compile_jit(gpu_target);
        i3_gpu.realize(s_old_g);
