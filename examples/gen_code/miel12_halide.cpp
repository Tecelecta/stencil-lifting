...
// miel12 kernel
Func targetFunction(Buffer<float, 3> u_old,
                   Buffer<float, 3> tendency,
                   Buffer<float, 3> rom,
                   Buffer<float, 2> muu_old,
                   Buffer<float, 2> muu_new,
                   Buffer<float, 1> c1,
                   Buffer<float, 1> c2,
                   Buffer<float, 2> msfuy,
                   Buffer<float, 1> rdzw) {
    Var i("i"), k("k"), j("j");
    Func rt("advect_u_implicit");
    Expr dt_rk(.375f);
    ...
    rt(i,k,j) = dt_rk * tendency(i,k,j) * msfuy(i,j)
                - (c1(k)*muu_old(i,j)+c2(k))*(at*u_old(i,km1,j) + btmp*u_old(i,k,j) + ct*u_old(i,kp1,j));
    return rt;
}

extern "C" {
    void advect_u_implicit_(
        float* rt,
        const float* u_old, const float* tendency, const float* rom,
...
int main(int argc, char** argv)
{
    ...
    Buffer<float, 2> msfuy({in_d1_range, in_d3_range}, "msfuy"), msfuy_g(msfuy),
                     muu_old({in_d1_range, in_d3_range}, "muu_old"), muu_old_g(muu_old),
                     muu_new({in_d1_range, in_d3_range}, "muu_new"), muu_new_g(muu_new);
    
    Buffer<float, 3> u_old({in_d1_range, in_d2_range, in_d3_range}, "u_old"), u_old_g(u_old),
                     tendency({in_d1_range, in_d2_range, in_d3_range}, "tendency"), tendency_g(tendency),
                     rom({in_d1_range+1, in_d2_range+1, in_d3_range}, "rom"), rom_g(rom);
    ...
    // --------------------------- miel12 kernel --------------------------
    printf("miel12 kernel start!\n");

    auto res_cpu = targetFunction(u_old, tendency, rom, muu_old, muu_new, c1, c2, msfuy, rdzw);
    ...
    try {
    i3_cpu.realize(u_old);
    i3_cpu.realize(tendency);
    i3_cpu.realize(rom);
    i2_cpu.realize(msfuy);
    i2_cpu.realize(muu_old);
    i2_cpu.realize(muu_new);
    i1_cpu.realize(rdzw);
    i1_cpu.realize(c1);
    i1_cpu.realize(c2);
    ...
    if ( with_gpu )
    {
        auto res_gpu = targetFunction(u_old_g, tendency_g, rom_g, muu_old_g, muu_new_g, c1_g, c2_g, msfuy_g, rdzw_g);
        res_gpu.gpu_tile(d1,d2,d3,d1o,d2o,d3o,d1i,d2i,d3i,8,8,4)
                .compile_jit(gpu_target);
        i3_gpu.realize(u_old_g);
        i3_gpu.realize(tendency_g);
        i3_gpu.realize(rom_g);
        i2_gpu.realize(msfuy_g);
        i2_gpu.realize(muu_old_g);
        i2_gpu.realize(muu_new_g);
