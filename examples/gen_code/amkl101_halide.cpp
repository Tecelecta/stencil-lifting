...
extern "C" {
    void advec_mom_kernel_loop101_(int *j,
                                   int *k,
                                   double *mass_flux_x,
                                   double *node_flux,
...
int main(int argc, char** argv)
{
    ...
    Buffer<double,2> node_flux_base({x_range + 6, y_range + 6}, "node_flux");
    
    Buffer<double,2> node_flux_cpu({x_range + 5, y_range + 2}, "node_flux");
    
    Buffer<double,2> node_flux_gpu({x_range + 5, y_range + 2}, "node_flux");

    // --------------------------- advec_mom_kernel_loop101 kernel --------------------------
    // printf("advec_mom_kernel_loop101 kernel start!\n");
    Func cpu_fn("cpu_fn");
    cpu_fn(j, k) = Expr(0.25) * (mass_flux_x(j, k + 1) + mass_flux_x(j, k + 2) + 
                                  mass_flux_x(j + 1, k + 1) + mass_flux_x(j + 1, k + 2));
    cpu_fn.parallel(k);
    ...
    for (int y = 0; y < mass_flux_x.height(); y++) {
        for (int x = 0; x < mass_flux_x.width(); x++) {
            mass_flux_x(x, y) = ((((x + y) % 13) + 0.1) / 13.0) * 10.0;
        }
    }

    zero_cpu.realize(node_flux_base);

    // Calling baseline Fortran
    double *mass_flux_x_ = mass_flux_x.get()->begin();
    double *node_flux_ = node_flux_base.get()->begin();
    
    int j_dummy = 0, k_dummy = 0;
    advec_mom_kernel_loop101_(&j_dummy, &k_dummy,
                              mass_flux_x_, node_flux_, 
                              &x_max, &x_min, &y_max, &y_min);
    
    // Calling halide cpu
    try {
        zero_cpu.realize(node_flux_cpu);
        cpu_fn.realize(node_flux_cpu);
    ...
    for (int k_idx = 0; k_idx < y_range + 2; k_idx++) {
        for (int j_idx = 0; j_idx < x_range + 5; j_idx++) {
            int fortran_j = j_idx + x_min - 2;
            int fortran_k = k_idx + y_min;
            int cpp_j = j_idx;
            int cpp_k = k_idx + 2;
            
            double diff = abs(node_flux_base(cpp_j, cpp_k) - node_flux_cpu(j_idx, k_idx));
            
            if (diff >= 1e-10)
            {
                if (errors < 10) {
                    // printf("node_flux [%d, %d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
                    //         j_idx, k_idx, node_flux_base(cpp_j, cpp_k), node_flux_cpu(j_idx, k_idx), diff);
                }
                errors++;
            }
            correctness += diff;
    ...
    // printf("Total errors for node_flux: %d, sum of differences: %e\n", errors, correctness);
    ...
    if (with_gpu)
    {
        // building gpu func
        Func gpu_fn("gpu_fn");
        gpu_fn(j, k) = Expr(0.25) * (mass_flux_x_g(j, k + 1) + mass_flux_x_g(j, k + 2) + 
                                      mass_flux_x_g(j + 1, k + 1) + mass_flux_x_g(j + 1, k + 2));
        ...
        // warmup
        try {
            zero_gpu.realize(node_flux_gpu);
            gpu_fn.realize(node_flux_gpu);
        } catch ( RuntimeError &e ) {
            std::cerr << e.what();
            return -1;
        }
