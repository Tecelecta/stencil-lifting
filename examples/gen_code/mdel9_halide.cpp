...
inline Func targetFunction(Buffer<double, 3>& v,
                           Buffer<double, 2>& msfvx,
                           const Expr jts, const Expr jte)
{
    Var i("i"), k("k"), j("j");
    Func res("cal_deform_and_div");

    res(i, k, j) = select(j == jts || j == jte, 
        Expr(0.),
        v(i,k,j) / msfvx(i,j)
    );
    return res;
...
extern "C" {
    void cal_deform_and_div_(double *hat, 
                             double *msfvx,
                             double *v,
...
int main(int argc, char** argv)
{
    ...
    Buffer<double,3> v({i_range+1, k_range+1, j_range+1}, "v"), v_g(v);
    Buffer<double,2> msfvx({i_range+1, j_range+1}, "msfvx"), msfvx_g(msfvx);

    // baseline input
    Buffer<double,3> out_base({i_range+1, k_range+1, j_range+1}, "hatavg");
    // halide cpu output
    Buffer<double,3> out_cpu({i_range+1, k_range+1, j_range+1}, "hatavg");
    // halide gpu output
    Buffer<double,3> out_gpu({i_range+1, k_range+1, j_range+1}, "hatavg");

    // --------------------------- cal_deform_and_div kernel --------------------------
    printf("mdel3 kernel start!\n");

    // building baseline stencil
    Func cpu_fn = targetFunction(v, msfvx, Expr(jts), Expr(jte));
    ...
    // IO initialization
    init3_cpu.realize(v);
    init2_cpu.realize(msfvx);

    zero_cpu.realize(out_base);

    // Calling baseline
    cal_deform_and_div_(out_base.get()->begin(), 
                        msfvx.get()->begin(), 
                        v.get()->begin(), 
    ...
    printf("cal_deform_and_div legacy code average cost time: %lfms\n\n",cost_time/times*1000);
    ...
    printf("cal_deform_and_div lifted cpu average cost time: %lfms\n\n",cost_time/times*1000);


    if (with_gpu)
    {
        // building gpu func
        Func gpu_fn = targetFunction(v_g, msfvx_g, Expr(jts), Expr(jte));
        ...
        init3_gpu.realize(v_g);
        init2_gpu.realize(msfvx_g);
        ...
        timespec_diff(t2, t1, elapsed);
        cost_time += toSec(elapsed);
        printf("cal_deform_and_div lifted gpu average cost time: %lfms\n\n",cost_time/times*1000);
    }
