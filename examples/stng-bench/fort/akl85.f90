SUBROUTINE accelerate_kernel_loop85(stepbymass,viscosity,xarea,xvel1,x_max,x_min,y_max,y_min)
INTEGER :: j
INTEGER :: k
INTEGER :: x_max
INTEGER :: x_min
...
DO k = y_min, y_max + 1
DO j = x_min, x_max + 1
xvel1(j,k) = xvel1(j,k) - stepbymass(j,k) * &
             (xarea(j,k) * (viscosity(j,k) - viscosity(j - 1,k)) + &
