                           ...
                           const Expr y_min, const Expr y_max)
{
    ...
    // Fortran: density1(j,k) = density0(j,k)
    ...
    target(j, k) = select(roi, 
        density0(j - x_min + 2, k - y_min + 2),
        density1(j, k));
    return target;
}

extern "C" {
    void revert_kernel_loop87_(
...
int main(int argc, char** argv)
{
    ...
    Buffer<double, 2> density1_base({x_range + 5, y_range + 5}, "density1");
    Buffer<double, 2> density1_cpu({x_range + 1, y_range + 1}, "density1");
    Buffer<double, 2> density1_gpu({x_range + 1, y_range + 1}, "density1");

    // --------------------------- revert_kernel_loop87 kernel --------------------------
    
    // building cpu func
    Func cpu_fn = targetFunction("revert_kernel_loop87_cpu", 
                                density1_cpu, density0,
    ...
    // Calling halide cpu
    try {
        zero2_cpu.realize(density1_cpu);
        cpu_fn.realize(density1_cpu);
    ...
    for (int k_idx = 0; k_idx < y_range + 1; k_idx++) {
        for (int j_idx = 0; j_idx < x_range + 1; j_idx++) {
            ...
            // density1_base index for Fortran density1(j_idx+x_min, k_idx+y_min) is (j_idx+x_min-x_min+2, k_idx+y_min-y_min+2) = (j_idx+2, k_idx+2)
            
            double diff = abs(density1_base(j_idx + 2, k_idx + 2) - density1_cpu(j_idx, k_idx));
            if (diff >= 1e-10)
            {
                if (errors < 10) {
                    printf("output [%d, %d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
                           j_idx, k_idx, density1_base(j_idx + 2, k_idx + 2), density1_cpu(j_idx, k_idx), diff);
                }
                errors++;
            }
            correctness_1 += diff;
    ...
    {
        // building gpu func
        Func gpu_fn = targetFunction("revert_kernel_loop87_gpu", 
                                    density1_gpu, density0_g,
                                    Expr(x_min), Expr(x_max), 
                                    Expr(y_min), Expr(y_max));
        ...
        // warmup
        try {
            zero2_gpu.realize(density1_gpu);
            gpu_fn.realize(density1_gpu);
        } catch (RuntimeError &e) {
            std::cerr << e.what();
            return -1;
        }
