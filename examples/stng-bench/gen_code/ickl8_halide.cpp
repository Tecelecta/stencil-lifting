                           ...
                           const Expr x_min, const Expr x_max)
{
    ...
    // Fortran: vertexdx(j) = dx
    target(j) = dx;
    return target;
}

extern "C" {
    void initialise_chunk_kernel_loop8_(
        const int *j,
        const double *dx,
        double *vertexdx,
...
int main(int argc, char** argv)
{
    ...
    Func init1_cpu = init_nonzero<1>("init1", x_range + 6 >= 8);
    Func zero1_cpu = set_zero<1>("zero1", x_range + 6 >= 8);
    ...
    // Array dimensions:
    // vertexdx: (x_min-2):(x_max+3) -> x_range + 6
    Buffer<double, 1> vertexdx_base(x_range + 6, "vertexdx"), vertexdx_g(vertexdx_base);
    Buffer<double, 1> vertexdx_cpu(x_range + 6, "vertexdx");
    Buffer<double, 1> vertexdx_gpu(x_range + 6, "vertexdx");

    // --------------------------- initialise_chunk_kernel_loop8 kernel --------------------------
    
    double dx = 0.25;
    
    // building cpu func
    Func cpu_fn = targetFunction("initialise_chunk_kernel_loop8_cpu", 
                                vertexdx_cpu, Expr(dx),
    ...
    // IO initialization
    init1_cpu.realize(vertexdx_base);

    // Calling baseline
    int dummy_j = 0;
    double *vertexdx_ = vertexdx_base.get()->begin();
    initialise_chunk_kernel_loop8_(
        &dummy_j,
        &dx,
        vertexdx_,
        &x_max, &x_min);
    ...
    // Calling halide cpu
    try {
        zero1_cpu.realize(vertexdx_cpu);
        cpu_fn.realize(vertexdx_cpu);
    ...
    int errors = 0;
    for (int j_idx = 0; j_idx < x_range + 6; j_idx++) {
        // Fortran loop: j from x_min-2 to x_max+3
        // This corresponds to j_idx from 0 to x_range+5
        
        double diff = abs(vertexdx_base(j_idx) - vertexdx_cpu(j_idx));
        if (diff >= 1e-10)
        {
            if (errors < 10) {
                printf("output [%d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
                       j_idx, vertexdx_base(j_idx), vertexdx_cpu(j_idx), diff);
            }
            errors++;
        }
        correctness_1 += diff;
    ...
    {
        // building gpu func
        Func gpu_fn = targetFunction("initialise_chunk_kernel_loop8_gpu", 
                                    vertexdx_gpu, Expr(dx),
                                    Expr(x_min), Expr(x_max));
        gpu_fn.gpu_tile(j, bj, tj, 256)
              .compile_jit(gpu_target);
        
        // GPU IO initialization
        vertexdx_g.copy_from(vertexdx_base);
        
        // warmup
        try {
            zero1_gpu.realize(vertexdx_gpu);
            gpu_fn.realize(vertexdx_gpu);
        } catch (RuntimeError &e) {
            std::cerr << e.what();
            return -1;
        }
