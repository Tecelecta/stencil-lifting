                    ...
                    Buffer<double,2>& yvel1)
{
    ...
    Func yvel1_cnvrt("yvel1_convert");
    yvel1_cnvrt(d1,d2) = yvel1(d1+2,d2+2);
    Func yarea_cnvrt("yarea_convert");
    yarea_cnvrt(d1,d2) = yarea(d1+2,d2+2);

    Func result_func("yvel1_fn");
    result_func(d1,d2) = yvel1_cnvrt(d1,d2)-stepbymass_cnvrt(d1,d2) 
                        * (yarea_cnvrt(d1,d2) * (viscosity_cnvrt(d1,d2)-viscosity_cnvrt(d1,d2-1))
                         + yarea_cnvrt(d1-1,d2) * (viscosity_cnvrt(d1-1,d2)-viscosity_cnvrt(d1-1,d2-1)));

    return result_func;
}

extern "C" {
    void accelerate_kernel_loop86_(
        double *viscosity,
        double *stepbymass,
        double *yarea,
        double *yvel1,
...
int main(int argc, char **argv)
{
    ...
    Buffer<double, 2> yvel1({d1_base_range+6,d2_base_range+6}, "yvel1"), yvel1_g(yvel1);

    // --------------------------- accelerate_kernel_loop86 kernel --------------------------
    printf("accelerate_kernel_loop86 start!\n");
    ...
    Func result_func_cpu = targetFunction(yarea, viscosity, stepbymass, yvel1);
    ...
    if (with_gpu)
    {
        Func result_func_gpu = targetFunction(yarea, viscosity, stepbymass, yvel1);
