...
void advec_cell_kernel_loop92_(
    double *post_vol,
    double *pre_vol,
    double *vol_flux_x,
    double *volume,
...
int main(int argc, char** argv)
{
    ...
    Func i1_cpu("i1_cpu");
    int input1_d1_base_range = d1_base_range+5; // change along with different kernels
    ...
    Buffer<double> vol_flux_x({d1_base_range+5,d2_base_range+4}, "vol_flux_x"),
                   vol_flux_x_g(vol_flux_x) ;
    ...
    // --------------------------- advec_cell_kernel_loop92 kernel -------------------------- // change along with different kernels
    printf("advec_cell_kernel_loop92 start!\n"); // change along with different kernels
    ...
    // ------- result1_func --------
    Func result2_func_cpu("pre_vol_fn");
    result2_func_cpu(d1,d2) = volume(d1,d2)+vol_flux_x(d1+1,d2)-vol_flux_x(d1,d2);
    ...
    // ------- result2_func --------
    Func result1_func_cpu("post_vol_fn");
    ...
    if (with_gpu)
    {
        ...
        result2_func_gpu(d1,d2) = volume_g(d1,d2)+vol_flux_x_g(d1+1,d2)-vol_flux_x_g(d1,d2);
