...
inline Func targetFunction(const std::string& funcName,
                           ...
                           const Buffer<double, 2>& density1,
                           ...
                           const Expr y_min, const Expr y_max)
{
    ...
    // Fortran: density0(j,k) = density1(j,k)
    ...
    target(j, k) = select(roi, 
        density1(j - x_min + 2, k - y_min + 2),
        density0(j, k));
    return target;
}

extern "C" {
    void reset_field_kernel_loop109_(
        double *density0,
        double *density1,
...
int main(int argc, char** argv)
{
    ...
    // Array dimensions:
    // density0, density1: (x_min-2):(x_max+2), (y_min-2):(y_max+2) -> (x_range + 5) × (y_range + 5)
    Buffer<double, 2> density1({x_range + 5, y_range + 5}, "density1"), density1_g(density1);
    Buffer<double, 2> density0_base({x_range + 5, y_range + 5}, "density0");
    Buffer<double, 2> density0_cpu({x_range + 1, y_range + 1}, "density0");
    Buffer<double, 2> density0_gpu({x_range + 1, y_range + 1}, "density0");

    // --------------------------- reset_field_kernel_loop109 kernel --------------------------
    
    // building cpu func
    Func cpu_fn = targetFunction("reset_field_kernel_loop109_cpu", 
                                density0_cpu, density1,
    ...
    // IO initialization
    init2_cpu.realize(density1);
    init2_cpu.realize(density0_base);

    // Calling baseline
    double *density0_ = density0_base.get()->begin();
    double *density1_ = density1.get()->begin();
    reset_field_kernel_loop109_(
        density0_,
        density1_,
    ...
    // Calling halide cpu
    try {
        zero2_cpu.realize(density0_cpu);
        cpu_fn.realize(density0_cpu);
    ...
    for (int k_idx = 0; k_idx < y_range + 1; k_idx++) {
        for (int j_idx = 0; j_idx < x_range + 1; j_idx++) {
            ...
            // density0_base index for Fortran density0(j_idx+x_min, k_idx+y_min) is (j_idx+x_min-x_min+2, k_idx+y_min-y_min+2) = (j_idx+2, k_idx+2)
            
            double diff = abs(density0_base(j_idx + 2, k_idx + 2) - density0_cpu(j_idx, k_idx));
            if (diff >= 1e-10)
            {
                if (errors < 10) {
                    printf("output [%d, %d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
                           j_idx, k_idx, density0_base(j_idx + 2, k_idx + 2), density0_cpu(j_idx, k_idx), diff);
                }
                errors++;
            }
            correctness_1 += diff;
    ...
    {
        // building gpu func
        Func gpu_fn = targetFunction("reset_field_kernel_loop109_gpu", 
                                    density0_gpu, density1_g,
                                    Expr(x_min), Expr(x_max), 
                                    Expr(y_min), Expr(y_max));
        ...
        // warmup
        try {
            zero2_gpu.realize(density0_gpu);
            gpu_fn.realize(density0_gpu);
        } catch (RuntimeError &e) {
            std::cerr << e.what();
            return -1;
        }
