                    ...
                    Buffer<double,2>& xvel0)
{
    ...
    Func xvel0_cnvrt("xvel0_convert");
    xvel0_cnvrt(d1,d2) = xvel0(d1+2,d2+2);
    Func xarea_cnvrt("xarea_convert");
    xarea_cnvrt(d1,d2) = xarea(d1+2,d2+2);

    Func result_func("xvel1_fn");
    result_func(d1,d2) = xvel0_cnvrt(d1,d2)-stepbymass_cnvrt(d1,d2) 
                        * (xarea_cnvrt(d1,d2) * (pressure_cnvrt(d1,d2)-pressure_cnvrt(d1-1,d2))
                        + xarea_cnvrt(d1,d2-1) * (pressure_cnvrt(d1,d2-1)-pressure_cnvrt(d1-1,d2-1)));

    return result_func;
}

extern "C" {
    void accelerate_kernel_loop83_(
        double *pressure,
        double *stepbymass,
        double *xarea,
        double *xvel0,
...
int main(int argc, char **argv)
{
    ...
    Buffer<double, 2> xvel0({d1_base_range+6,d2_base_range+6}, "xvel0"), xvel0_g(xvel0);

    // --------------------------- accelerate_kernel_loop83 kernel --------------------------
    printf("accelerate_kernel_loop83 start!\n");
    ...
    Func result_func_cpu = targetFunction(xarea, pressure, stepbymass, xvel0);
    ...
    i1_cpu.realize(xvel0);
    i1_cpu.realize(xarea);
    
    z_cpu.realize(out_base);
    accelerate_kernel_loop83_(
        pressure.get()->begin(),
    ...
    if (with_gpu)
    {
        Func result_func_gpu = targetFunction(xarea, pressure, stepbymass, xvel0);
