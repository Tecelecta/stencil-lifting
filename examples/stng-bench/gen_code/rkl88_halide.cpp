                           ...
                           const Expr y_min, const Expr y_max)
{
    ...
    // Fortran: energy1(j,k) = energy0(j,k)
    ...
    target(j, k) = select(roi, 
        energy0(j - x_min + 2, k - y_min + 2),
        energy1(j, k));
    return target;
}

extern "C" {
    void revert_kernel_loop88_(const int* j, const int* k,
...
int main(int argc, char** argv)
{
    ...
    Buffer<double, 2> energy0({x_range + 5, y_range + 5}, "energy0"), energy0_g(energy0);
    Buffer<double, 2> energy1_base({x_range + 5, y_range + 5}, "energy1");
    Buffer<double, 2> energy1_cpu({x_range + 1, y_range + 1}, "energy1");
    Buffer<double, 2> energy1_gpu({x_range + 1, y_range + 1}, "energy1");

    // --------------------------- revert_kernel_loop88 kernel --------------------------
    
    // building cpu func
    Func cpu_fn = targetFunction("revert_kernel_loop88_cpu", 
                                energy1_cpu, energy0,
    ...
    init2_cpu.realize(energy1_base);

    // Calling baseline
    int dummy_j = 0, dummy_k = 0;
    double *energy0_ = energy0.get()->begin();
    double *energy1_ = energy1_base.get()->begin();
    revert_kernel_loop88_(
        &dummy_j, &dummy_k,
        energy0_,
        energy1_,
    ...
    // Calling halide cpu
    try {
        zero2_cpu.realize(energy1_cpu);
        cpu_fn.realize(energy1_cpu);
    ...
    for (int k_idx = 0; k_idx < y_range + 1; k_idx++) {
        for (int j_idx = 0; j_idx < x_range + 1; j_idx++) {
            ...
            // energy1_base index for Fortran energy1(j_idx+x_min, k_idx+y_min) is (j_idx+x_min-x_min+2, k_idx+y_min-y_min+2) = (j_idx+2, k_idx+2)
            
            double diff = abs(energy1_base(j_idx + 2, k_idx + 2) - energy1_cpu(j_idx, k_idx));
            if (diff >= 1e-10)
            {
                if (errors < 10) {
                    printf("output [%d, %d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
                           j_idx, k_idx, energy1_base(j_idx + 2, k_idx + 2), energy1_cpu(j_idx, k_idx), diff);
                }
                errors++;
            }
            correctness_1 += diff;
    ...
    {
        // building gpu func
        Func gpu_fn = targetFunction("revert_kernel_loop88_gpu", 
                                    energy1_gpu, energy0_g,
                                    Expr(x_min), Expr(x_max), 
                                    Expr(y_min), Expr(y_max));
        ...
        // warmup
        try {
            zero2_gpu.realize(energy1_gpu);
            gpu_fn.realize(energy1_gpu);
        } catch (RuntimeError &e) {
            std::cerr << e.what();
            return -1;
        }
