...
int main(int argc, char** argv)
{
    ...
    // Array dimensions:
    // xvel0, xvel1: (x_min-2):(x_max+3), (y_min-2):(y_max+3) -> (x_range + 6) × (y_range + 6)
    Buffer<double, 2> xvel1({x_range + 6, y_range + 6}, "xvel1"), xvel1_g(xvel1);
    Buffer<double, 2> xvel0_base({x_range + 6, y_range + 6}, "xvel0");
    Buffer<double, 2> xvel0_cpu({x_range + 2, y_range + 2}, "xvel0");
    Buffer<double, 2> xvel0_gpu({x_range + 2, y_range + 2}, "xvel0");

    // --------------------------- reset_field_kernel_loop111 kernel --------------------------
    
    // building cpu func
    Func cpu_fn = targetFunction("reset_field_kernel_loop111_cpu", 
                                xvel0_cpu, xvel1,
    ...
    // Calling halide cpu
    try {
        zero2_cpu.realize(xvel0_cpu);
        cpu_fn.realize(xvel0_cpu);
    ...
    for (int k_idx = 0; k_idx < y_range + 2; k_idx++) {
        for (int j_idx = 0; j_idx < x_range + 2; j_idx++) {
            ...
            // xvel0_base index for Fortran xvel0(j_idx+x_min, k_idx+y_min) is (j_idx+x_min-x_min+2, k_idx+y_min-y_min+2) = (j_idx+2, k_idx+2)
            
            double diff = abs(xvel0_base(j_idx + 2, k_idx + 2) - xvel0_cpu(j_idx, k_idx));
            if (diff >= 1e-10)
            {
                if (errors < 10) {
                    printf("output [%d, %d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
                           j_idx, k_idx, xvel0_base(j_idx + 2, k_idx + 2), xvel0_cpu(j_idx, k_idx), diff);
                }
                errors++;
            }
            correctness_1 += diff;
    ...
    {
        // building gpu func
        Func gpu_fn = targetFunction("reset_field_kernel_loop111_gpu", 
                                    xvel0_gpu, xvel1_g,
                                    Expr(x_min), Expr(x_max), 
                                    Expr(y_min), Expr(y_max));
        ...
        // warmup
        try {
            zero2_gpu.realize(xvel0_gpu);
            gpu_fn.realize(xvel0_gpu);
        } catch (RuntimeError &e) {
            std::cerr << e.what();
            return -1;
        }
