...
extern "C" {
    void advec_mom_kernel_loop103_(double *mom_flux,
...
int main(int argc, char** argv)
{
    ...
    // --------------------------- advec_mom_kernel_loop103 kernel --------------------------
    // printf("advec_mom_kernel_loop103 kernel start!\n");
    Func cpu_fn("cpu_fn");
    cpu_fn(j, k) = (vel1_cpu(j, k) * node_mass_pre(j + 2, k + 2) + mom_flux(j + 1, k + 2) - mom_flux(j + 2, k + 2)) / node_mass_post(j + 2, k + 2);
    cpu_fn.parallel(k);
    ...
    for (int y = 0; y < vel1_base.height(); y++) {
        for (int x = 0; x < vel1_base.width(); x++) {
            vel1_base(x, y) = ((((x + y + 3) % 13) + 0.1) / 13.0) * 10.0;
            vel1_cpu(x, y) = vel1_base(x, y);
        }
    }
    ...
    advec_mom_kernel_loop103_(mom_flux_, node_mass_post_, node_mass_pre_, vel1_, 
    ...
    for (int k_idx = 0; k_idx < y_range + 2; k_idx++) {
        for (int j_idx = 0; j_idx < x_range + 2; j_idx++) {
            ...
            double diff = abs(vel1_base(cpp_j, cpp_k) - vel1_cpu(j_idx, k_idx));
            
            if (diff >= 1e-10)
            {
                if (errors < 10) {
                    // printf("vel1 [%d, %d] = Fortran: %lf | Halide: %lf | diff: %e\n", 
                    //         j_idx, k_idx, vel1_base(cpp_j, cpp_k), vel1_cpu(j_idx, k_idx), diff);
                }
                errors++;
            }
            correctness += diff;
    ...
    if (with_gpu)
    {
        // building gpu func
        Func gpu_fn("gpu_fn");
        gpu_fn(j, k) = (vel1_gpu(j, k) * node_mass_pre_g(j + 2, k + 2) + mom_flux_g(j + 1, k + 2) - mom_flux_g(j + 2, k + 2)) / node_mass_post_g(j + 2, k + 2);
