...
inline Func targetFunction(const std::string& funcName,
                           ...
                           const Buffer<double, 1>& vertexy,
                           const Expr y_min, const Expr y_max)
{
    Var k("k");
    Func target(funcName);
    
    // Fortran: celly(k) = 0.5 * (vertexy(k) + vertexy(k + 1))
    // Direct implementation without ROI since we're computing on the full array
    target(k) = Expr(0.5) * (vertexy(k) + vertexy(k + 1));
    return target;
}

extern "C" {
    void initialise_chunk_kernel_loop12_(int *k,
                                        double *celly,
                                        double *vertexy,
                                        const int* y_max,
                                        const int* y_min);
...
int main(int argc, char** argv)
{
    ...
    Func init1_cpu = init_nonzero<1>("init", y_range + 6 >= 8);
    ...
    // celly: (y_min-2):(y_max+2) -> y_range + 5
    // vertexy: (y_min-2):(y_max+3) -> y_range + 6
    Buffer<double,1> celly_base(y_range + 5, "celly");
    Buffer<double,1> vertexy(y_range + 6, "vertexy");
    Buffer<double,1> celly_cpu(y_range + 5, "celly");
    Buffer<double,1> celly_gpu(y_range + 5, "celly");
    Buffer<double,1> vertexy_g(vertexy);

    // --------------------------- initialise_chunk_kernel_loop12 kernel --------------------------
    int k_dummy = 0;

    // building baseline stencil
    Func cpu_fn = targetFunction("initialise_chunk_kernel_loop12_cpu",
                                celly_cpu, vertexy,
    ...
    // IO initialization
    init1_cpu.realize(vertexy);
    zero_cpu.realize(celly_base);
    zero_cpu.realize(celly_cpu);

    // Calling baseline Fortran
    double *celly_ = celly_base.get()->begin();
    double *vertexy_ = vertexy.get()->begin();
    initialise_chunk_kernel_loop12_(&k_dummy, celly_, vertexy_, &y_max, &y_min);
    ...
    {
        // building gpu func
        Func gpu_fn = targetFunction("initialise_chunk_kernel_loop12_gpu",
                                    celly_gpu, vertexy_g,
                                    Expr(y_min), Expr(y_max));
        gpu_fn.gpu_tile(k, bk, tk, 256).compile_jit(gpu_target);
        
        // GPU IO initialization
        vertexy_g.copy_from(vertexy);
