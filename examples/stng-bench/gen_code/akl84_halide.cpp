...
Func targetFunction(Buffer<double,2>& yarea, 
                    Buffer<double,2>& pressure, 
                    Buffer<double,2>& stepbymass,
                    Buffer<double,2>& yvel0)
{
    Var d1("j"), d2("k");
    Func pressure_cnvrt("pressure_convert");
    pressure_cnvrt(d1,d2) = pressure(d1+2,d2+2);
    Func stepbymass_cnvrt("stepbymass_convert");
    stepbymass_cnvrt(d1,d2) = stepbymass(d1+2,d2+2);
    Func yvel0_cnvrt("yvel0_convert");
    yvel0_cnvrt(d1,d2) = yvel0(d1+2,d2+2);
    Func yarea_cnvrt("yarea_convert");
    yarea_cnvrt(d1,d2) = yarea(d1+2,d2+2);

    Func result_func("yvel1_fn");
    result_func(d1,d2) = yvel0_cnvrt(d1,d2)-stepbymass_cnvrt(d1,d2) 
                        * (yarea_cnvrt(d1,d2) * (pressure_cnvrt(d1,d2)-pressure_cnvrt(d1,d2-1))
                         + yarea_cnvrt(d1-1,d2) * (pressure_cnvrt(d1-1,d2)-pressure_cnvrt(d1-1,d2-1)));

    return result_func;
}

extern "C" {
    void accelerate_kernel_loop84_(
        double *pressure,
        double *stepbymass,
        double *yarea,
        double *yvel0,
...
int main(int argc, char **argv)
{
    ...
    Buffer<double, 2> pressure({d1_base_range+5,d2_base_range+5}, "pressure"), pressure_g(pressure);
    Buffer<double, 2> stepbymass({d1_base_range+6,d2_base_range+6}, "stepbymass"), stepbymass_g(stepbymass);
    Buffer<double, 2> yarea({d1_base_range+5,d2_base_range+6}, "yarea"), yarea_g(yarea);
    Buffer<double, 2> yvel0({d1_base_range+6,d2_base_range+6}, "yvel0"), yvel0_g(yvel0);

    // --------------------------- accelerate_kernel_loop84 kernel --------------------------
    printf("accelerate_kernel_loop84 start!\n");
    ...
    Func result_func_cpu = targetFunction(yarea, pressure, stepbymass, yvel0);
    ...
    i1_cpu.realize(pressure);
    i1_cpu.realize(stepbymass);
    i1_cpu.realize(yvel0);
    i1_cpu.realize(yarea);
    
    z_cpu.realize(out_base);
    accelerate_kernel_loop84_(
        pressure.get()->begin(),
    ...
    if (with_gpu)
    {
        Func result_func_gpu = targetFunction(yarea, pressure, stepbymass, yvel0);
        result_func_gpu.gpu_tile(d1,d2,d1o,d2o,d1i,d2i,8,8);
        result_func_gpu.compile_jit(gpu_target);

        i1_gpu.realize(pressure_g);
        i1_gpu.realize(stepbymass_g);
        i1_gpu.realize(yvel0_g);
        i1_gpu.realize(yarea_g);
