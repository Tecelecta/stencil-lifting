...
extern "C" {
    void advec_mom_kernel_loop98_(double *post_vol,
                                  double *pre_vol, 
                                  double *vol_flux_x,
                                  double *vol_flux_y,
                                  double *volume,
...
int main(int argc, char** argv)
{
    ...
    // --------------------------- advec_mom_kernel_loop98 kernel --------------------------
    // printf("advec_mom_kernel_loop98 kernel start!\n");

    // post_vol(j,k) = volume(j,k) + vol_flux_x(j + 1,k) - vol_flux_x(j,k)
    Func cpu_fn_post("cpu_fn_post");
    cpu_fn_post(j, k) = volume(j, k) + vol_flux_x(j + 1, k) - vol_flux_x(j, k);
    ...
    cpu_fn_pre(j, k) = post_vol_cpu(j, k) + vol_flux_y(j, k + 1) - vol_flux_y(j, k);
    ...
    advec_mom_kernel_loop98_(post_vol_, pre_vol_, 
    ...
    if (with_gpu)
    {
        ...
        gpu_fn_post(j, k) = volume_g(j, k) + vol_flux_x_g(j + 1, k) - vol_flux_x_g(j, k);
        gpu_fn_post.gpu_tile(j, k, bj, bk, tj, tk, 8, 8)
                   .compile_jit(gpu_target);

        Func gpu_fn_pre("gpu_fn_pre");
        gpu_fn_pre(j, k) = post_vol_gpu(j, k) + vol_flux_y_g(j, k + 1) - vol_flux_y_g(j, k);
